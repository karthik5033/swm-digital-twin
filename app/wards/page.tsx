...
function getScoreColor(score: number) {
  if (score <= 40) return { bg: 'bg-rose-50', border: 'border-rose-200', text: 'text-rose-600', chart: '#e11d48' }; // Red
  if (score <= 70) return { bg: 'bg-amber-50', border: 'border-amber-200', text: 'text-amber-600', chart: '#d97706' }; // Amber
  return { bg: 'bg-emerald-50', border: 'border-emerald-200', text: 'text-emerald-600', chart: '#059669' }; // Green
}
...
export default function WardsPage() {
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-slate-50 text-slate-900 font-sans p-4 md:p-8 transition-colors">
      <div className="max-w-[1400px] mx-auto">
        
        {/* TOP: SUMMARY STRIP */}
        <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-6 mb-8 flex flex-col md:flex-row items-center justify-between gap-6 transition-colors">
          <div className="flex items-center gap-6 flex-wrap">
            <div>
              <h1 className="text-3xl font-black text-slate-900 tracking-tight">HSR Layout Waste Management</h1>
              <p className="text-sm text-slate-500 font-medium">Real-time metrics for HSR Layout sectors and zones</p>
            </div>
          ...
          <div className="bg-rose-50 border border-rose-100 text-rose-600 px-5 py-3 rounded-xl flex items-center gap-3 shadow-sm shrink-0">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-[40%] flex flex-col gap-6">
            <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm min-h-[500px] flex flex-col transition-colors">
              <h2 className="text-lg font-extrabold text-slate-900 mb-2">HSR Layout Geographic Grid</h2>
            ...
            <AnimatePresence mode="popLayout">
              {selectedWardId && (
                <motion.div 
                  ...
                  exit={{ opacity: 0, y: 20 }}
                  className="bg-white border border-slate-200 rounded-2xl p-6 shadow-md transition-colors"
                >
                  {isLoading ? (
                      ...
                      <div className="flex flex-col gap-4">
                        <div className="flex justify-between w-full items-start">
                          <div>
                            ...
                            <span className="text-sm font-bold text-slate-500 uppercase tracking-wide">{w.zone} Zone</span>
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 mt-2">
                          <div className="bg-slate-50 border border-slate-200 p-3 rounded-lg flex flex-col">
                            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Dump Risk</span>
                            <span className="text-lg font-bold text-slate-800">{(w.dumpRisk * 100).toFixed(0)}%</span>
                          </div>
                          <div className="bg-slate-50 border border-slate-200 p-3 rounded-lg flex flex-col">
                            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Methane Int.</span>
                            <span className="text-lg font-bold text-slate-800">{(w.methaneIntensity * 100).toFixed(0)}%</span>
                          </div>
                          <div className="bg-slate-50 border border-slate-200 p-3 rounded-lg flex flex-col">
                            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Efficiency</span>
                            <span className="text-lg font-bold text-slate-800">{(w.routeEfficiency * 100).toFixed(0)}%</span>
                          </div>
                          <div className="bg-slate-50 border border-slate-200 p-3 rounded-lg flex flex-col">
          ...
          <div className="w-full lg:w-[60%] flex flex-col bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden min-h-[500px] transition-colors">
            <div className="p-6 border-b border-slate-100">
              <h2 className="text-xl font-extrabold text-slate-900 mb-4">HSR Layout Sector Metrics</h2>
              
              {/* Filter Bar */}
              <div className="flex flex-col sm:flex-row gap-4 mb-2">
                {filteredWards.length > 0 && (
                  <button onClick={() => setFilteredWards([])} className="bg-teal-50 text-teal-700 font-bold px-4 py-2.5 rounded-xl border border-teal-200 text-sm shadow-sm whitespace-nowrap hover:bg-teal-100 transition-colors">
                    Clear AI Filter ✕
                  </button>
                )}
                <div className="relative flex-1">
                  ...
                  <input 
                    ...
                    className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:bg-white transition-all shadow-sm"
                  />
                </div>
                <select
                  value={filterZone}
                  onChange={(e) => { setFilterZone(e.target.value); setCurrentPage(1); }}
                  className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-600 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:bg-white shadow-sm cursor-pointer"
                >
            ...
            <div className="overflow-x-auto flex-1">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-100 text-xs uppercase tracking-wider text-slate-500 font-extrabold transition-colors">
                  ...
                  {isLoading ? (
                    ...
                    return (
                      <tr 
                        ...
                        className={`cursor-pointer transition-colors ${isSelected ? 'bg-teal-50 hover:bg-teal-100/50' : 'hover:bg-slate-50'}`}
                      >
                        <td className="px-4 py-3 border-l-4" style={{ borderLeftColor: isSelected ? '#0d9488' : 'transparent' }}>
                          <span className={`font-bold ${isSelected ? 'text-teal-700' : 'text-slate-700'}`}>{w.name}</span>
                        </td>
            ...
            {/* Pagination */}
            {totalPages > 1 && (
              <div className="p-4 border-t border-slate-100 bg-slate-50 flex items-center justify-between transition-colors">
            ...
            return (
              <motion.button
                ...
                className={`
                  ...
                  ${isSelected ? 'ring-2 ring-teal-500 ring-offset-2 z-10' : 'hover:shadow-md hover:border-slate-300 z-0'}
                  bg-white border-slate-200
                `}
              >
                <div className="flex flex-col items-start min-w-0">
                  <span className={`text-[10px] font-bold uppercase truncate w-full text-left leading-tight ${isSelected ? 'text-teal-700' : 'text-slate-600'}`}>
                    {shortName}
                  </span>
                </div>
                ...
                <div className={`absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full border-2 border-white ${w.score <= 40 ? 'bg-rose-500' : w.score <= 70 ? 'bg-amber-500' : 'bg-emerald-500'}`} />
              </motion.button>
            );
          })
        )}
