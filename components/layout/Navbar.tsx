  ...
  return (
    <nav className="relative flex items-center justify-between p-4 border-b border-slate-800 bg-slate-950/80 backdrop-blur-md sticky top-0 z-50">
      ...
      <div className="flex items-center gap-2 md:gap-6">
        ...
        <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-800 border-slate-700 border font-medium text-xs">
        ...
        <button 
          onClick={toggleTheme} 
          className="flex items-center justify-center w-10 h-10 rounded-full hover:bg-slate-800 transition-colors text-slate-300"
            ...
            <div key={link.href} className="flex items-center">
              {link.label === 'Report' && (
                <button 
                  ...
                  className="mr-2 ml-4 bg-slate-800 text-slate-300 hover:bg-slate-700 w-7 h-7 rounded-full flex items-center justify-center transition-colors text-xs font-black shadow-sm"
            ...
            <AnimatePresence>
              {featuresOpen && (
                <motion.div 
                  ...
                  className="absolute top-full right-0 mt-2 w-80 bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden z-[60]"
                >
                    ...
                    {FEATURES_LINKS.map(link => (
                      <Link 
                        ...
                        className={`group flex items-start gap-4 p-3 rounded-xl transition-all duration-200 ${
                          isActive(link.href) ? 'bg-slate-800' : 'hover:bg-slate-800/60'
                        }`}
                      >
                        <div className="text-2xl mt-0.5 grayscale group-hover:grayscale-0 transition-all">{link.icon}</div>
                        <div>
                          <div className={`text-sm font-bold mb-0.5 ${isActive(link.href) ? 'text-teal-400' : 'text-slate-200 group-hover:text-teal-400'} transition-colors`}>{link.label}</div>
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="absolute top-full left-0 w-full bg-slate-900 border-b border-slate-800 md:hidden overflow-hidden z-50 origin-top"
            >
              <div className="p-4 flex flex-col gap-2">
                <div className="text-xs font-bold text-slate-500 uppercase px-2 mb-1 tracking-wider">Pages</div>
                {MAIN_LINKS.map(link => (
                  <Link key={link.href} href={link.href} className="px-4 py-3 rounded-xl text-slate-300 font-semibold hover:text-teal-400 hover:bg-slate-800">
                    {link.label}
                  </Link>
                ))}
                ...
                {FEATURES_LINKS.map(link => (
                  <Link key={link.href} href={link.href} className="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-300 font-semibold hover:text-teal-400 hover:bg-slate-800">
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative max-w-2xl w-full max-h-[90vh] overflow-y-auto bg-slate-900 border border-slate-700 rounded-3xl shadow-2xl z-10 p-6 sm:p-8"
            >
              <button 
                onClick={() => setIsModalOpen(false)}
                className="absolute top-6 right-6 w-8 h-8 flex items-center justify-center rounded-full bg-slate-800 text-slate-500 hover:text-white transition-colors"
              >
                ✕
              </button>
              ...
              <div className="overflow-x-auto border border-slate-800 rounded-2xl mb-8">
                <table className="w-full text-left text-sm whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-slate-800 font-medium text-slate-400">
                    {[
                      ...
                      <tr key={i} className="hover:bg-slate-800/30">
                        ...
                        <td className="px-5 py-3 font-mono text-teal-400">{row.val}</td>
